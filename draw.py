...
for i in range(num_step):
    ...
    # if i % output_interval == 0 and i > 0:  # 去掉第0轮的判断
    #     # 保存生成的图片
...
plt.plot(epochs, content_losses_history, label='Content Loss', color='red')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.savefig(os.path.join(output_folder, 'loss_curve1.png'))
plt.show()