...
class StyleLoss(nn.Module):
    ...
    def forward(self, input):
        ...
        # 获取多尺度特征
        scales = [0.5, 0.75, 1.0, 1.25, 1.5, 2]  # 可根据需要调整
        style_loss = 0
...
optimizer = optim.LBFGS([input_parm],lr=0.1)
num_step = 700
...
for i in range(num_step):
    ...
    #     return style_score + content_score
        # 在循环迭代之前定义总变差正则化的权重
    total_variation_weight = 0.0003  # 调整为适当的值
    ...
    def closure():
        ...
        total_variation_loss = total_variation_weight * (
            torch.sum(torch.abs(input_parm[:, :, :, :-1] - input_parm[:, :, :, 1:])) +
            torch.sum(torch.abs(input_parm[:, :, :-1, :] - input_parm[:, :, 1:, :]))
        )

        total_loss = style_score + content_score + total_variation_loss
        total_loss.backward()
...
output_path = os.path.join(output_folder, 'lenablue总变差+style1-1.5.png')
output_img = out_put.detach().cpu().squeeze(0)
